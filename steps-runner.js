...
export async function run(stage) {
    ...
    const errorStepsFile = path.join(ACTION_STEP_TEMP_DIR, 'steps-error.json');
    fs.appendFileSync(errorStepsFile, ''); // ensure the file does exist
    fs.readFileSync(errorStepsFile).toString().split('\n').filter((line) => !!line)
        .forEach((stepIndex) => {
    ...
    if (stage === 'Pre') {
        await startAct(steps, githubToken);
    }

    // --- tail act log file
    const actLogTail = new TailFile(actLogPath, {startPos: stage === 'Pre' ? 0 : null});
    ...
    function completeStep(stepIndex, result) {
        ...
        if (result) {
            ...
            if (result === 'error') {
                fs.appendFileSync(errorStepsFile, stepIndex + EOL);
            }
        }
...
async function startAct(steps, githubToken) {
